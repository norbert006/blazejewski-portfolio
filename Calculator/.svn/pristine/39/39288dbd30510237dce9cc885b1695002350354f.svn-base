...
public class RevPolishCalc {
   ...
   * @throws InvalidExpressionException - throws a custom exception message when the user provides
   *         an invalid expression.
   */
  public float evaluate(String expression) throws InvalidExpressionException {
    // If the expression is blank, for now it returns any odd value.
      ...
      for (String token : expression.split("\\s+")) {
        switch (token) {
          ...
          case "/":
            ...
            break;
          default:
            // If the token is not a +, -, * or /, the value is pushed
            // onto the stack.
